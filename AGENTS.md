...
state/               runtime records and signals; gitignored
  ...
  .secondmate-relaunch-<id> .secondmate-relaunch-bound-<id>   per-mate durable auto-relaunch ledger (epoch + attempt/relaunched/failed rows) and parked-bound marker, both written only through bin/fm-secondmate-liveness-lib.sh and the watcher's liveness tick; removed by teardown, safe to delete (a fresh death episode then counts from zero)
