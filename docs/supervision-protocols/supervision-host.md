...
{codex}    While the record exists each checkpoint lasts at least `FM_CODEX_WATCH_CHECKPOINT_AWAY` seconds (default 3600), so a captain message waits until the checkpoint returns unless the captain interrupts it.
