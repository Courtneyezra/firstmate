...
| session isolation survives a hangup | a runner armed from a launcher holding a session of its own is in neither that session nor its group, and a `SIGHUP` delivered to every member of that session - which kills the launching leader, so the delivery is proved rather than assumed - leaves the runner and its polling child alive and the interrupted round still completes into a normal capture |
| a runner death is not silent | a runner killed with its whole process group inside its source command is announced as one durable `check` wake naming the source and saying the round captured nothing: the owner guard publishes it from outside the dead runner's session, `reconcile` publishes the same one when the guard is killed first, neither announces twice, the orphaned runner marker that would fail this home's sweep preflight is cleared with it, and the source stays registered and is relaunched |
